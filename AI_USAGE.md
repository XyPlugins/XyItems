...
- 根据服主确认，AI 将强度实现为“当前品质内属性范围的加权平均”，而不是跨品质比较；每项属性先独立随机，所有参与属性到达各自 `max` 时为100%。
- 新增轻量 `StrengthDefinition` 配置解析。强度只在生成鉴定结果时计算，不创建定时器，不在物品栏扫描，因此不会增加常驻性能开销。
- 强度百分比和条码只作为生成时的最终 Lore；原始强度百分比与随机属性同时写入 XyCore NBT，避免重载后旧物品发生变化，也避免后续插件反向解析 Lore。
- 根据服主提出的“鉴定/净化/拯救/重构”可替换需求，新增 `identify.action-name`，并提供 `<identify.action>`、`<动作.名称>` 和 `{action}` 消息占位符。
- 新增 `items/Example/Chiyamopo.yml`，示范赤牙墨魄的 `净化` 动作、百分比强度和十格 `llllllllll` 条码表现。
- 未改变既有品质权重、锻造失败权重、XyCore NBT 身份验证和严格背包容量规则。
- 已执行 `gradlew.bat clean test --no-daemon`，5项测试全部通过；仍需在实际 Paper/Spigot 1.12.2 与 XyCore 环境进行联机鉴定测试。
...
- 根据服主确认的显示格式修复强度条：默认十格为 `l`，70% 的 Lore 原文为 `&clllllll&7lll`；渲染器会合并连续颜色段，避免重复写入颜色码。
- 强度条格数使用向上取整：任何大于0的强度至少有一格 `&c`，所以1% 为 `&cl&7lllllllll`，0% 才是全灰。
- 根据服主补充的配置语义，默认 `filled`/`empty` 只写 `&c`/`&7`，渲染器自动补十格 `l`；同时保留显式 `&cl`/`&7l` 的兼容写法。
